...
def A_volatiles(t):
    ...
    return  A_I_131(t) + A_I_133(t)
def H_Xe(t):
    return (A_Xe_133(t) * p.E_3 + A_Xe_135(t) * p.E_5) * (1e15)


# For volatiles, it is assumed only Iodine
def H_volatiles(t, filter):
    #return (A_Sr_90(t) * filter * p.E_sr + A_Te_132(t) * filter * p.E_te + A_I_131(t) * \
    # filter * p.E_I_1 + A_I_133(t) * filter * p.E_I_3 + A_Cs_137(t) * filter * p.E_Cs + \
    # A_Ba_140(t) * filter * p.E_Ba) * (1e15 * p.IR)
    return (A_I_131(t) * filter * p.E_I_1 + A_I_133(t) * filter * p.E_I_3) * (1e15 * p.IR)
